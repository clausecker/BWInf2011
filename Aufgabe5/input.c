...
int read_int(FILE *stream, int *integer)
{
	...
	*integer = c - '0';
	...
	ungetc(c, stream); // put the last non-numeric character back to the stream
...
int read_partnerships(FILE *fpartnerships, bool *partnerships, int *num_partnerships, struct CITY *cities, int num_cities)
{
	*num_partnerships = 0;
	while(1)
	{
		...
		{
			int i;
			for(i = 0; i < num_cities && (city_indexes[0] == -1 || city_indexes[1] == -1); i++)
			{
				if(city_indexes[0] == -1 && !strcmp(city_names[0], cities[i].name))
					city_indexes[0] = i;
				if(city_indexes[1] == -1 && !strcmp(city_names[1], cities[i].name))
					city_indexes[1] = i;
			}
		}
